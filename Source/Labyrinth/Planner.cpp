...
Planner::Planner(WorldState* currentws, WorldState* g, const TArray<UGOAPAction*>& a)
{
	currentWorld = currentws;
	goal = g;
	actions = a;
}

GOAPNode Planner::lowestFinList(const TArray<GOAPNode>& opList)
{
	GOAPNode node;
...
TArray<GOAPNode> Planner::getAdjacent(GOAPNode current, const TArray<UGOAPAction*>& vActions, APawn* p)
{
...
TArray<UGOAPAction*> Planner::generatePlan(APawn* p)
{
...
void Planner::addAction(UGOAPAction* a)
{
	this->actions.Push(a);
}

WorldState Planner::getGoal()
{
	return *goal;
}
void Planner::setGoal(WorldState* g)
{
	this->goal = g;
}
WorldState Planner::getCurrentWorld()
{
	return *currentWorld;
}
void Planner::setCurrentWorld(WorldState* w)
{
	this->currentWorld = w;
}

int Planner::getMaxDepth() {
	return maxDepth;
}

void Planner::setMaxDepth(int md) {
	maxDepth = md;
}
